...
This portfolio shows the construction of three different visualizations 
...
To get the data into the correct format for the starship visualization, we again created a `tibble` of the data from `repurrrsive`'s `sw_starships` dataset and unlisted the columns that were stored as lists. We converted all numeric variables to be numeric. Consumables (i.e., the amount of food and life-preserving resources the ship can hold) was stored in a rather odd format: the variable had both a number and a unit (e.g., 1 month, 3 years, 1 week, 5 days...). To address this, I `separate`d the number from the unit and then converted the unit to the number of days it represented. I then multiplied the number and the unit together, to provide the number of days the starship had consumables for. 
...
planets %>%
  ...
  # add labels indicating y-axis units
    geom_text(aes(x = 0, y =   0, label =   "STAR"), color = "deeppink4") +
    geom_text(aes(x = 0, y =  .5, label =  ".5 AU"), color = "deeppink4") +
    geom_text(aes(x = 0, y =   1, label =   "1 AU"), color = "deeppink4") +
    geom_text(aes(x = 0, y = 1.5, label = "1.5 AU"), color = "deeppink4") +
    geom_text(aes(x = 0, y =   2, label =   "2 AU"), color = "deeppink4") +
    ...
    # customize using my theme
    my_theme() +
    # drop axis text and axis titles
    theme(axis.text  = element_blank(),
          axis.title = element_blank()) +
    # add text identifying only those planetss from the original trilogy
    geom_text_repel(data = . %>% filter(!is.na(orig_tril)), aes(label = name),
                    color              = "#00ddff",
